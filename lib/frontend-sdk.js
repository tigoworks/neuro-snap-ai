...
class NeuroSnapAPI {
  ...
  async getAnalysisResult(surveyId) {
    console.log('📤 获取分析结果:');
    console.log('  endpoint: /api/analysis-result/user/' + surveyId);
    console.log('  method: GET');
    
    return this.request(`/api/analysis-result/user/${surveyId}`);
  }
  ...
  async pollAnalysisResult(surveyId, options = {}) {
    ...
    for (let i = 0; i < maxAttempts; i++) {
      try {
        ...
        const response = await fetch(`${this.baseURL}/api/analysis-result/user/${surveyId}`, {
          headers: {
            'Content-Type': 'application/json',
            'X-Frontend-Key': this.apiKey,
            'User-Agent': 'NeuroSnap-Frontend/1.0.0'
        ...
        if (response.status === 429) {
          ...
          const waitTime = retryAfter ? parseInt(retryAfter) * 1000 : Math.min(5000 * Math.pow(2, i), 30000);
        ...
        if (response.status === 404) {
          const currentInterval = Math.min(
            initialInterval * Math.pow(1.4, i),
            20000 // 最大间隔20秒
          );
          
          console.log(`⏳ 分析进行中 (404)，${Math.round(currentInterval / 1000)}秒后重试... (${i + 1}/${maxAttempts})`);
        ...
        if (result.success && result.data.status === 'not_found') {
          ...
          console.log(`⏳ 分析进行中 (not_found)，${Math.round(currentInterval / 1000)}秒后重试... (${i + 1}/${maxAttempts})`);
          await new Promise(resolve => setTimeout(resolve, currentInterval));
          continue;
        }
        ...
        // 错误重试策略：指数退避
        const errorInterval = Math.min(10000 * (i + 1), 60000);
        console.warn(`🔄 ${errorInterval / 1000}秒后重试...`);
        await new Promise(resolve => setTimeout(resolve, errorInterval));
      }
    }

    throw new Error(`分析超时：已尝试 ${maxAttempts} 次，请稍后手动查询结果`);
  }
  ...
  async getAnalysisHistory(userId, limit = 10, offset = 0) {
    console.log('📤 获取分析历史:');
    console.log('  endpoint: /api/analysis-result/user/' + userId + '/history');
    console.log('  method: GET');
    console.log('  params: { limit:', limit, ', offset:', offset, '}');
    
    return this.request(`/api/analysis-result/user/${userId}/history?limit=${limit}&offset=${offset}`);
  }
  ...
  async getAnalysisSummary(userId) {
    console.log('📤 获取分析摘要:');
    console.log('  endpoint: /api/analysis-result/user/' + userId + '/summary');
    console.log('  method: GET');
    
    return this.request(`/api/analysis-result/user/${userId}/summary`);
  }
