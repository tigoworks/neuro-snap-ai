...
class NeuroSnapAPI {
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 429) {
          throw new Error(`HTTP 429: 请求过于频繁，请稍后重试`);
        } else if (response.status === 404) {
          throw new Error(`HTTP 404: 分析结果未找到，可能还在生成中`);
        } else {
  ...
  async pollAnalysisResult(surveyId, options = {}) {
    const {
      maxAttempts = 20,      // 最大尝试次数（减少）
      interval = 5000,       // 轮询间隔（毫秒）- 增加到5秒
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (error.message && error.message.includes('429')) {
          const backoffDelay = Math.min(interval * Math.pow(2, attempt - 1), 30000); // 指数退避，最大30秒
          console.log(`🚦 请求频率过高，等待 ${backoffDelay/1000} 秒后重试...`);
